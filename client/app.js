const app = new Vue({
    ...
    data:{
        ...
        currentChat:'',
        disconnected:true,
        socket:io("https://murmuring-badlands-72180.herokuapp.com"),
        child:'',
        parent:'',
        newChatM:false,
        newFriend:false
    ...
    methods:{
        ...
        requestUser(userID,friend){
            ...
            axios.post('https://murmuring-badlands-72180.herokuapp.com/request',{"from":userID,"for":friend})
            .then(res=>{
                this.requesting=false;
        ...
        createUser(username){
            if(username.length<5){
                ...
                this.choosing=true;
                axios.post('https://murmuring-badlands-72180.herokuapp.com/user',{'username':username})
                .then(res=>{
        ...
        acceptRequest(request,index){
           ...
           axios.post('https://murmuring-badlands-72180.herokuapp.com/chat',payload)
           .then(res=>{
              const id1 = res.data.between[0]; const id2 = res.data.between[1];
        ...
        declineRequest(id,index){
            this.requests.splice(index,1);
            axios.delete(`https://murmuring-badlands-72180.herokuapp.com/request/${id}`)
            .then(res=>{
        ...
        sendMessage(message){
            if(message.length>0){
                ...
                this.socket.emit('messageSent',{chat:this.currentChat,id1:party1ID,id2:party2ID});
                axios.post(`https://murmuring-badlands-72180.herokuapp.com/message/${this.currentChat._id}`,payload)
                .then(res=>{
        ...
        updateChat(updatedChat){
            ...
            this.chats = this.chats.map(chat => (chat._id === updatedChat._id) ? updatedChat : chat);
        }
    },
    mounted(){
        ...
        if(this.userID){
             axios.get(`https://murmuring-badlands-72180.herokuapp.com/user/${this.userID}`)
            .then(res=>{
                 if(!res.data){
                    this.loading=false; 
                    ...
                    this.userID = res.data._id;
                    this.registerSocket(this.userID);
                    axios.get(`https://murmuring-badlands-72180.herokuapp.com/request/${this.userID}`)
                    .then(res=>{
                        this.requests=res.data;
                        axios.get(`https://murmuring-badlands-72180.herokuapp.com/chat/${this.userID}`)
                        .then(res=>{
                            this.chats=res.data;
